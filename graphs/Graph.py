...
class Graph:
    ...
    @staticmethod
    def paint_compare(compare_path):
        compare = np.loadtxt(compare_path)
        x = range(51)
        plt.figure(1)
        plt.rcParams['font.sans-serif'] = ['SimHei']
        plt.plot(x, compare[0, :], linestyle='-')
        plt.plot(x, compare[1, :], linestyle='--')
        plt.plot(x, compare[2, :], linestyle=':')
        plt.plot(x, compare[3, :], marker='o', linestyle='-', markersize=3)
        plt.plot(x, compare[4, :], marker='^', linestyle='-', markersize=3)
        plt.plot(x, compare[5, :], marker='*', linestyle='-', markersize=3)
        label = ["union_order=1", "union_order=2", "union_order=3", u"接近度中心性", u"度中心性", u"特征向量中心性"]
...
if __name__ == "__main__":
    ...
    # compare_path = '../LTM/netscience_compare.txt'
    compare_path = '../LTM/er_compare.txt'
    Graph.paint_compare(compare_path)